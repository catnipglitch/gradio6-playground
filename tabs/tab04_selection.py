...
def _show_values(
    ...
    lines = [
        ...
        f"**Dropdown (multiselect)**: {multi_dropdown}",
        f"**ColorPicker**: `{color}`",
    ]
    return "\n\n".join(lines)
...
def build_tab() -> None:
    with gr.Tab("Tab 4: 選択・カラー入力"):
        gr.Markdown(
            """## Tab 4: 選択・カラー入力コンポーネント
